	...
	for rownum in range(data_file_sheet.nrows):
		...
		for b in range(len(data_file_sheet.row_values(rownum))):
			...
			if fcs_section_check is True and row_column is 2:
				...
				fcs_section_number = automiatic_fcs_section_number(filename, fcs_section_auto_number)
...
def automiatic_fcs_section_number(filename, automiatic_fcs_section_number):
	data_file = xlrd.open_workbook("Sections_input.xls")
	data_file_sheet = data_file.sheet_by_index(0)
	print "DEBUG: == automiatic_fcs_section_number == ", automiatic_fcs_section_number
	cell = data_file_sheet.cell(automiatic_fcs_section_number-1,find_val_in_workbook(filename))

	cell_value = cell.value
	print "DEBUG: == cell_value == ", cell_value

	return num(cell_value)
